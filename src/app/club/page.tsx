  ...
  return (
    <div className="flex flex-col min-h-[100dvh] text-foreground pb-24 pt-safe relative z-10">
      ...
      <div className="flex justify-between items-center px-4 pb-4">
        <div className="flex items-center gap-3">
          <Link href={inClub ? `/club/${userData?.clubMembers[0]?.clubId}` : "/club/create"}>
            <div className="w-12 h-12 rounded-[16px] bg-primary flex items-center justify-center shadow-[0_0_15px_rgba(204,255,0,0.3)]">
              <CrewLogo size={40} className="text-black" />
            </div>
          </Link>
          <div className="flex flex-col">
            <h1 className="text-2xl font-black tracking-tighter uppercase leading-none">CREW</h1>
          </div>
        </div>
      </div>
      {/* Franchise CTA */}
      <div className="px-4 mb-4">
        <Link href={inClub ? `/club/${userData?.clubMembers[0]?.clubId}` : "/club/create"}>
          <div className="w-full bg-card border border-border rounded-[24px] p-5 flex items-center justify-between relative overflow-hidden group">
            <div className="absolute inset-0 bg-gradient-to-r from-primary/10 to-transparent pointer-events-none group-hover:from-primary/20 transition-colors"></div>
            <div className="relative z-10">
              <h3 className="font-black uppercase tracking-tight text-lg mb-1">{inClub ? "Мой клуб" : "Создать клуб"}</h3>
              <p className="text-xs text-muted">{inClub ? "Перейти к управлению и событиям клуба" : "Собери свою беговую банду и врывайся в топ"}</p>
            </div>
            <div className="w-10 h-10 rounded-full bg-background border border-border flex items-center justify-center text-primary relative z-10 group-hover:scale-110 transition-transform">
